...
int main()
{
    int n, count = 0, pos = 0, current_time, inc, smallest;
    float avg_wt = 0, avg_tt = 0;
    int burst_time[10], arrival_time[10], priority[10], turnaround_time[10], waiting_time[10], process[10], done_process[10], remain_time[10], end_time, start_time[10], response_time[10];
    ...
    for (current_time = 0; count != n; current_time++)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            if (arrival_time[i] <= current_time && done_process[i] == 0 && remain_time[i] > 0 && priority[i] < priority[smallest])
            {
                smallest = i;
        ...
        if (remain_time[smallest] == 0 && done_process[smallest] == 0)
        {
            ...
            printf("\nP%d \t\t %d \t\t\t %d \t\t\t %d \t\t\t %d \t\t\t\t %d \t\t\t %d", process[smallest], arrival_time[smallest], burst_time[smallest], priority[smallest], turnaround_time[smallest], waiting_time[smallest], response_time[smallest]);
        }
    }
